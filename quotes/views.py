...
from .forms import QuoteModelForm, QuoteNoteForm, QuoteReviewForm, QuoteFormset
...
class QuoteCreateView(LoginRequiredMixin, generic.CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.POST.get('remove_product'):
            print("Removed")
            return redirect("quotes:quote-list")
...
class QuoteListReview(LoginRequiredMixin, generic.CreateView):
    template_name = "quotes/quote_list_review.html"
    form_class = QuoteReviewForm
    
    def get_context_data(self, **kwargs):
        context = super(QuoteListReview, self).get_context_data(**kwargs)
        context['object_list'] = int(Quote.objects.filter(id=self.kwargs['pk']).values_list('id',flat=True)[0])
        context['review_list'] = QuoteReviews.objects.filter(quote_id=self.kwargs['pk'])
        
        try:
            context['review_update_list'] = list(QuoteReviews.objects.filter(author_id=self.request.user.id, quote_id=self.kwargs['pk']).values_list('id',flat=True))
        except IndexError:
            pass
    ...
    def form_valid(self, form):
        form.instance.author_id = self.request.user.id
        form.instance.quote_id = self.kwargs['pk']
        return super(QuoteListReview, self).form_valid(form)

    def get_success_url(self):
        return self.request.path

class QuoteDeleteReview(LoginRequiredMixin, generic.DeleteView):
    model = QuoteReviews
    template_name = "quotes/quote_delete_review.html"

    def dispatch(self, request, *args, **kwargs):
        obj = self.get_object()
        if obj.author_id != self.request.user.id:
            raise PermissionDenied("User can't delete this question.")
        return super(QuoteDeleteReview, self).dispatch(request, *args, **kwargs)
